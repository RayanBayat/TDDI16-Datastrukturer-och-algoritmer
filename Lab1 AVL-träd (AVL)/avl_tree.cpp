...
void AVL_Tree_Node<Comparable>::insert(const Comparable &x, Node_Pointer &t) {
    ...
    if (x < t->element) {
       //  std::cout << "nu pekas det på(insert) " << t->element <<"\n";
         // cout << "(före insättning) left height: " << node_height(t->left) << ". Right height : "<< node_height(t->right)<< "\n";
        insert(x, t->left);
 
        if (node_height(t->left) - node_height(t->right) == 2)
            if (x < t->left->element){
                 std::cout << "single_rotate_with_left_child (insert) " << t->element <<"\n";
                single_rotate_with_left_child(t);
              

                }
            else{
                std::cout << "dubbel_rotate_with_left_child (insert) " << t->element <<"\n";
      ...
      //  cout << "(efter insetning) left height: " << node_height(t->left) << ". Right height : "<< node_height(t->right) <<"\n";
    } else if (t->element < x) {
        // std::cout << "nu pekas det på(insert) " << t->element <<"\n";
         // cout << "(före insättning)left height: " << node_height(t->left) << ". Right height : "<< node_height(t->right) <<"\n";
        insert(x, t->right);
        ...
        if (node_height(t->right) - node_height(t->left) == 2)
            if (t->right->element < x){
             //    std::cout << "single_rotate_with_right_child (insert) " << t->element <<"\n";
                single_rotate_with_right_child(t);
                }
            else{
               //  std::cout << "dubbel_rotate_with_right_child (insert) " << t->element <<"\n";
...
//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
// @Rayan Jag lade till simple remove funktionen från andra filen och kopplade det
// till resten av projectet
template <typename Comparable>
void AVL_Tree_Node<Comparable>::remove(const Comparable &x, Node_Pointer &t) {
    if (t == nullptr) {
        throw AVL_Tree_error("Siffran finns inte");
    ...
    if (x < t->element) {
        ...
        if ((node_height(t->right) - node_height(t->left) >= 2))
        {
            ...
            if ((t->right->element < x) || (node_height((t->right)->right) - node_height((t->right)->left)) >= 0 ){
              //   std::cout << "single_rotate_with_right_child (remove) " << t->element <<"\n";
                single_rotate_with_right_child(t);
                }

            else{
               //  std::cout << "dubbel_rotate_with_right_child (remove) " << t->element <<"\n";
                double_rotate_with_right_child(t);
                }
            }
        else
        ...
        remove(x, t->right);
            ...
            if (node_height(t->left) - node_height(t->right) >= 2)
            {
            if ((x < t->left->element)||(node_height((t->left)->left) - node_height((t->left)->right)) >= 0){
               //  std::cout << "single_rotate_with_left_child (remove) " << t->element <<"\n";
                single_rotate_with_left_child(t);
              

               }
            else{
                //std::cout << "dubbel_rotate_with_left_child (remove) " << t->element <<"\n";
        ...
        if (t->left != nullptr && t->right != nullptr) {
            ...
            if (t->left == nullptr)
            {
             //  cout << "right element :"<< t->right->element << "\n";

                t = t->right;
            }
            else{
               // cout << "left element :"<<t->left->element << "\n";
                t = t->left;
            }
          
            delete tmp;
           //   cout << "(remove) left height: " << node_height(t->left) << ". Right height : "<< node_height(t->right)<<"\n";
        }
    }
